...
describe("NFTCollection", () => {
  ...
  describe("Template deployment", () => {
    ...
    it("Should not be initializable once deployed", async () => {
      ...
      ).to.be.revertedWith("Cannot be initialized");
    });
  });
  ...
  describe("Deployment", () => {
    ...
    it("Should fail when tokens per mint is non-zero", async () => {
      await expect(
        deployInstance(
          "NFTCollection",
  ...
  describe("Contract version", () => {
    ...
    it("Should be set to 1_04_00", async () => {
      expect(await collection.VERSION()).to.equal(1_04_00);
    });
  });
  ...
  describe("Configuration", () => {
    ...
    describeVariable("publicMintPrice", (it) => {
      ...
      it.freezesOn(
        ...
        ethers.utils.parseEther(`0.${random(7000)}`)
      );
    });
    ...
    describeVariable("publicMintPriceFrozen", (it) => {
      ...
      it.freezesOn("publicMintPriceFrozen", false);
    });
    ...
    describeVariable("presaleMintPrice", (it) => {
      ...
      it.freezesOn(
        ...
        ethers.utils.parseEther(`0.${random(5000)}`)
      );
    });
    ...
    describeVariable("presaleMintPriceFrozen", (it) => {
      ...
      it.isUpdatableTo(false);
      it.isUpdatableTo(true);
      it.freezesOn("presaleMintPriceFrozen", false);
    });
    ...
    describeVariable("baseURI", (it) => {
      ...
      it("Should not be updatable if metadata is frozen", async () => {
        await updateConfig({ metadataUpdatable: false });

        await expect(
          updateConfig({ baseURI: `ipfs://${random(100000000)}/` })
    ...
    describeVariable("metadataUpdatable", (it) => {
      it.isRuntimeConfiguration();
      it.isInitializedTo(true);
      it.isUpdatableTo(false);
      it.isUpdatableTo(true);

      it("Should not let admins un-freeze metadata", async () => {
        await updateConfig({ metadataUpdatable: false });

        await expect(
          updateConfig({ metadataUpdatable: true })
        ).to.be.revertedWith("Cannot unfreeze metadata");
      });
